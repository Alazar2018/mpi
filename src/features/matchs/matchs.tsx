...
export default function Matchs() {
  ...
  useEffect(() => {
    matchesReq.send(() => getAllMatchs(), (res) => {
  ...
  const pendingMatches = (matchesReq.response?.matches || []).filter((match: Match) => match.status === 'pending');
  const recentMatches = (matchesReq.response?.matches || []).filter((match: Match) => match.status === 'completed');

  console.log(pendingMatches, recentMatches);
      ...
      <div className="grid grid-cols-3 gap-4">
        {(recentMatches ? ['', ...recentMatches] : [''])
          .map((match, idx) => {
            ...
            if (idx == 0) {
              return (
                <Link key={idx} to='/admin/matchs/new' className="h-[10.125rem] flex flex-col items-center justify-center gap-4 p-4 rounded-[1.5rem] border border-gray-1">
